...
```{r}
evenNumbers = seq(0,1000, by=2);
int_sum = 0;

for (i in 1:length(evenNumbers)){
  int_sum = int_sum + evenNumbers[i]
}
...
for (i in 1:dim(survey)[1]){
  for (j in 2:dim(survey)[2]){ # excludes the variable for participant 
  survey[i,j] = ifelse( (survey[i,j] >= 1 && survey[i, j] <= 5), survey[i, j], NA)} # for each item, if it isn't a valid answer, make it NA
} # first changes survey to exclude the invalid values

survey.clean = survey # survey.clean is just the new name of the survey
